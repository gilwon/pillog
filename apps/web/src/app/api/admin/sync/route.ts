  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      if (syncLogId) {
        try {
          if (full) {
            // 전체 동기화: 카운트만 조회 (44K+ 행 개별 기록은 DB 과부하 유발)
            const [{ count: newCount }, { count: updatedCount }] = await Promise.all([
              supabase.from('products').select('id', { count: 'exact', head: true })
                .gte('synced_at', syncStart).gte('created_at', syncStart),
              supabase.from('products').select('id', { count: 'exact', head: true })
                .gte('synced_at', syncStart).lt('created_at', syncStart),
            ])
            ...
            // 증분 동기화: ID 조회 + sync_log_products 개별 기록
            async function fetchProductIds(isNew: boolean): Promise<{ id: string }[]> {
              const all: { id: string }[] = []
              let from = 0
              const pageSize = 1000
              while (true) {
                let q = supabase.from('products').select('id').gte('synced_at', syncStart)
