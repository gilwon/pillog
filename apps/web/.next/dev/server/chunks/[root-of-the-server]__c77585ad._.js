...
async function GET(request) {
    try {
        ...
        // Fallback: pg_trgm 결과 없으면 functionality_tags 부분 일치 검색 (data + count 병렬)
        if (results.length === 0) {
            const [{ data: tagData }, { count: tagCount }] = await Promise.all([
                supabase.from('products').select('id, name, company, functionality_tags, shape').filter('functionality_tags::text', 'ilike', `%${q}%`).range(offset, offset + limit - 1),
                supabase.from('products').select('*', {
                    count: 'exact',
                    head: true
                }).filter('functionality_tags::text', 'ilike', `%${q}%`)
            ]);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c77585ad._.js.map