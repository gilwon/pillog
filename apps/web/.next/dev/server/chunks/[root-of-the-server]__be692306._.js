...
"[project]/apps/web/src/app/api/my/intake/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const { data: supplements, error: suppError } = await supabase.from('user_supplements').select(`
        id,
        product_id,
        daily_dose,
        product:products(
      ...
      `).eq('user_id', user.id);
        ...
        const { data: logs, error: logError } = await supabase.from('intake_logs').select('product_id, is_taken').eq('user_id', user.id).eq('taken_date', date).in('product_id', productIds);
        ...
        const items = supplements.map((s)=>{
            ...
            const rawIngredients = (product?.product_ingredients ?? []).filter((pi)=>pi.ingredient).sort((a, b)=>(b.amount ?? 0) - (a.amount ?? 0)).slice(0, 4).map((pi)=>({
...
async function POST(request) {
    try {
        ...
        if (!product_id || !(0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$lib$2f$utils$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["isValidUUID"])(product_id)) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__be692306._.js.map