...
"[project]/apps/web/src/app/api/products/compare/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        if (!ids.every((id)=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$lib$2f$utils$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["isValidUUID"])(id))) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: {
                    code: 'VALIDATION_ERROR',
                    message: '유효하지 않은 제품 ID가 포함되어 있습니다.',
                    status: 400
                }
            }, {
                status: 400
        ...
        // Fetch products
        const { data: products, error: productsError } = await supabase.from('products').select('id, name, company').in('id', ids).eq('is_active', true);
        ...
        const { data: allIngredients } = await supabase.from('product_ingredients').select(`
        product_id,
        amount,
        percentage_of_rdi,
        ...
        for (const pi of allIngredients || []){
            ...
            if (!ingredientMap.has(name)) {
                ingredientMap.set(name, {
                    ...
                    unit: ing.rdi_unit ?? null,
                    products: {}
                });
            }
            const item = ingredientMap.get(name);
            item.products[pi.product_id] = {
                amount: pi.amount ?? null,
                rdi_pct: pi.percentage_of_rdi ?? null
            };
        }
        const response = {
            ...
            comparison_table: Array.from(ingredientMap.values()).sort((a, b)=>a.category.localeCompare(b.category))
        };
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f7b3295a._.js.map